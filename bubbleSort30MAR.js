...
var sorted = unsortedArr.sort(function(a, b){
  return a-b;
});
console.log(sorted);
...
//and for each index look to the number (index-1)
//and if the number to the left is bigger than you swap
...
//function for one pass; sorted only one item
function sort(arr){
  for (var j = 1; j < arr.length; j++){ //
  //for (var i = 1; i < arr.length; i++){ 
    //go through entire loop
  for (var i = 1; i < arr.length - j; i++){ //goes though only the array that isn't sorted
    //number to the left is bigger than the number on the right
    if(arr[i-1] > arr[i]){
    //swap the number
    swap(arr, i-1, i) 
      }
